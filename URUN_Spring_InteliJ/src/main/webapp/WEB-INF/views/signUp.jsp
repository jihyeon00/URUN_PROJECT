...
<div class= "sign-form">
	<h2>회원가입</h2>
	<form action="/signUp" method="post" id="signUpForm" name="signUpck" onSubmit="return false">
		<!-- 사원아이디 -->
		<div class="signUp">
			<div class="sign-area" id="id-sign-area">
			 	<div class="signUpName">사원번호</div>
		    	<div id="id-area">
		    		<input type="text" name="MEMBER_ID" id="MEMBER_ID" autocomplete="off" required="required" autofocus="autofocus">
				</div>
			</div>
			<div class="signUpLabel" id="idLabel"></div>
			<div class="sign-area" id="pw-sign-area">
				<div class="signUpName">비밀번호</div>
				<div id="pw-area">
					<input type="password" name="MEMBER_PW" id="MEMBER_PW" autocomplete="off" required="required">
				</div>
			</div>
			<div class="signUpLabel" id="pwLabel"></div>
			<div class="sign-area" id="pwagree-sign-area">
			...
			<div class="signUpLabel" id="pwAgreeLabel"></div>
			<!-- 약관동의 -->
			<div class="text-area">
...
$(document).ready(function() {
	...
	$("#MEMBER_ID").on("focusout", function() {
    	...
    	$.ajax({
    		...
    		success : function(result) {
    			if (result == true) {
    				...
    				$("#idLabel").css("color", "green").text(MEMBER_ID+"는 회원가입 가능한 ID 입니다.");
    				setTimeout(function() {
    	  				$('#idLabel').css('display', 'none');
    	  			},3000);
    			} else{
    				$('#idLabel').css('display', 'block');
    				$("#idLabel").css("color", "red").text(MEMBER_ID+"는 회원가입 불가능한 ID 입니다.");
    				setTimeout(function() {
    	  				$('#idLabel').css('display', 'none');
    	  			},3000);
    			}
  	...
  	function prevCheckTextBox() {
  		if ($('#MEMBER_ID').val().length != 8||!$('#MEMBER_ID').val()) { // 이름 관련 dom
			$('#idLabel').css('display', 'none');
  			$('#idLabel').css('display', 'block');
  			$("#idLabel").css("color", "red").text('사원번호는 8자로 입력해 주세요.');
  			$('#MEMBER_ID').focus();		// 이름 입력 칸으로 포커스 이동
  			setTimeout(function() {
  				$('#idLabel').css('display', 'none');
  			},1500);
  			return false;
  		...
  		if (!$('#MEMBER_PW').val()||$('#MEMBER_PW').val().length< 6 || $('#MEMBER_PW').val().length > 10) {	// 패스워드 관련 dom
  			$('#pwLabel').css('display', 'block');
  			$("#pwLabel").css("color", "red").text('비밀번호를 6자로 입력해 주세요.');
  			$('#MEMBER_PW').focus();		// 패스워드 입력 칸으로 포커스 이동
  			setTimeout(function() {
  		...
  		if (!$('#signUpPasswordConfirm').val()) {	// 패스워드 관련 dom
  			$('#pwAgreeLabel').css('display', 'block');
  			$("#pwAgreeLabel").css("color", "red").text('패스워드를 한번 더 입력하세요.');
  			$('#signUpPasswordConfirm').focus();		// 패스워드 입력 칸으로 포커스 이동
  			setTimeout(function() {
  				$('#pwAgreeLabel').css('display', 'none');
  			},1500);
  			
  			return false;
  		}
   		if ($('#MEMBER_PW').val() != $('#signUpPasswordConfirm').val()) {	// 패스워드 관련 dom
   			$('#pwAgreeLabel').css('display', 'block');
  			$("#pwAgreeLabel").css("color", "red").text('패스워드가 맞지 않습니다.');
	 			$('#signUpPasswordConfirm').focus();		// 패스워드 입력 칸으로 포커스 이동
	 			setTimeout(function() {
	 				$('#pwAgreeLabel').css('display', 'none');
	  			},1500);
